...
if( !empty($_POST) ){
	...
	if( !empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['number']) && !empty($_POST['message']) && isValidEmail($_POST['email']) ){
		...
		foreach( $_POST as $key=>$val ){  // all form data
			if( $key != 'doing-via-ajax' ){
				$key = str_replace('_', ' ', $key);
				$key = str_replace('-', ' ', $key);
				$key = ucwords($key);
				if( strtolower($key) == 'message' ){ $val = nl2br($val); }
				$email_body .= '<tr>
					<td style="border: 1px solid #b5b5b5; padding: 5px;"><strong>' . trim($key) . ':</strong> </td>
					<td style="border: 1px solid #b5b5b5; padding: 5px;">' . $val . '</td>
				</tr>';
			}
		}
